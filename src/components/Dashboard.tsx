...
const StatusDot = ({ status, actual, target, direction }: any) => {
...
const Badge = ({ children, color = COLORS.red }: any) => (
  <span style={{
...
const Sparkline = ({ data, color = COLORS.redBright, width = 100, height = 28 }: any) => {
...
const genSparkData = (seed: string, count = 12, base = 50, volatility = 0.15, trendUp = true) => {
  const hash = seed.split("").reduce((a: number, c: string) => a + c.charCodeAt(0), 0);
  const rng = (i: number) => { const x = Math.sin(hash * 9301 + i * 49297) * 49297; return x - Math.floor(x); };
  const data = [];
  let val = base;
...
export default function ValveManCommandCenter() {
  ...
  const [calendarCustomItems, setCalendarCustomItems] = useState<any[]>([]);
  const [newCalItem, setNewCalItem] = useState({ title: "", cadence: "Monthly", type: "ceo" });
  const [showAddCal, setShowAddCal] = useState(false);
  const [aiCalSuggestions, setAiCalSuggestions] = useState<any>(null);
  ...
  const [customPrompts, setCustomPrompts] = useState<Record<string, string>>(() => {
    ...
    Object.entries(AI_SUGGESTIONS).forEach(([seg, items]) => {
      items.filter((s: any) => s.type === "ai").forEach((s: any) => {
        saved[`${seg}:${s.title}`] = s.prompt;
      });
    });
    return saved;
  });
  const [editingPromptKey, setEditingPromptKey] = useState<any>(null);
  const [editingPromptText, setEditingPromptText] = useState("");

  const startEditPrompt = (seg: string, suggestion: any) => {
  ...
  const getPrompt = (seg: string, suggestion: any) => customPrompts[`${seg}:${suggestion.title}`] || suggestion.prompt;
  ...
  const [lastInsightsRun, setLastInsightsRun] = useState<any>(null);
  ...
  // ─── ACTION TRACKING ───────────────────────────────────────────────────
  const [actionStates, setActionStates] = useState<Record<string, string>>({}); // key: "seg:title" => "done" | "snoozed" | "dismissed" | null
  const setActionState = (seg: string, title: string, state: any) => {
    setActionStates(prev => ({ ...prev, [`${seg}:${title}`]: state }));
  };
  const getActionState = (seg: string, title: string) => actionStates[`${seg}:${title}`] || null;

  // ─── KPI NOTES ─────────────────────────────────────────────────────────
  const [kpiNotes, setKpiNotes] = useState<Record<string, string>>({}); // key: kpi.id => string
  ...
  const getChannelGrade = (channelKpis: any[]) => {
  ...
  const generateTrendData = useCallback((kpi: any, range: string) => {
    const seed = kpi.id.split("").reduce((a: number, c: string) => a + c.charCodeAt(0), 0);
    const rng = (i: number) => {
  ...
  const deployAgent = async (suggestion: any) => {
  ...
  const acceptCalSuggestion = (suggestion: any) => {
    setCalendarCustomItems([...calendarCustomItems, { title: suggestion.title, cadence: suggestion.cadence, type: suggestion.type }]);
    setAiCalSuggestions(aiCalSuggestions.filter((s: any) => s.title !== suggestion.title));
  };
  ...
  // ─── KPI TABLE RENDERER ────────────────────────────────────────────────
  const renderKpiTable = (kpis: any[], title: string) => (
  ...
  const renderSuggestions = (segment: string) => {
    const items = (AI_SUGGESTIONS as any)[segment] || [];
    const aiItems = items.filter((s: any) => s.type === "ai");
    const ceoItems = items.filter((s: any) => s.type === "ceo");

    const ActionBar = ({ seg, title }: any) => {
    ...
    <>
      ...
      {(insights || insightsLoading) && (
        <div className="vm-card" style={{ ...styles.card, marginBottom: 24, borderColor: `${COLORS.blue}33`, borderLeft: `3px solid ${COLORS.blue}` }}>
          ...
          ) : insights && insights.map((item: any, i: number) => (
            <div key={i} style={{
                    ...
                    <td style={{ ...styles.td, textAlign: "center" }}>
                      {k.cadence !== "Daily" && (
                        <button
                          ...
                          onMouseEnter={e => { (e.target as HTMLElement).style.borderColor = COLORS.red; (e.target as HTMLElement).style.color = COLORS.redBright; }}
                          onMouseLeave={e => { (e.target as HTMLElement).style.borderColor = COLORS.border; (e.target as HTMLElement).style.color = COLORS.textMuted; }}
                        >📈</button>
                      )}
                    </td>
                  </tr>
      ...
      <>
        ...
        {aiCalSuggestions && aiCalSuggestions.length > 0 && (
          <div className="vm-card" style={{ ...styles.card, marginBottom: 20, borderColor: `${COLORS.blue}44` }}>
            ...
            {aiCalSuggestions.map((s: any, i: number) => (
              <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: `1px solid ${COLORS.border}22` }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  ...
                  <button style={styles.btnOutline} onClick={() => setAiCalSuggestions(aiCalSuggestions.filter((_: any, j: number) => j !== i))}>✗</button>
                </div>
              </div>
            ))}
          </div>
  ...
  const renderTrendModal = () => {
    ...
    const isPositive = isLower ? Number(changePct) < 0 : Number(changePct) > 0;
